  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [{
        role: 'user',
        content: [
          { type: 'text', text: 'Extract bill items in JSON: {items:[{name,price,quantity,category}],totalAmount,restaurantName}. Return ONLY JSON.' },
...
async function tryGemini(base64Data: string): Promise<{
  ...
  // Working models with v1beta API
      ...
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${process.env.GOOGLE_GEMINI_API_KEY}`,
        {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [
                { text: 'Extract bill as JSON: {"items":[{"name":"item","price":100,"quantity":1}],"totalAmount":500}. ONLY JSON.' },
  ...
  return { success: false, error: 'Gemini quota exceeded. Wait 24hrs or use OpenAI.' }
}
...
export async function parseBillFromImage(imageBase64: string): Promise<{
  ...
  return {
    success: false,
    error: geminiResult.error || 'No AI provider available. Add OpenAI key or wait for Gemini quota reset.',
    provider: 'none'
  }
}
...
export async function parseBillFromMultipleImages(images: string[]): Promise<{
  ...
  if (allItems.length === 0) {
    return { success: false, error: 'No items found', provider: provider || 'none' }
  }

  return {
    success: true,
    data: { items: allItems, totalAmount, restaurantName, confidence: 85 },
    provider
  }
}
