...
export default function BillPage() {
  ...
  useEffect(() => {
    if (shareId) {
      fetchBill();
      // FASTER POLLING: 5 seconds instead of 10
  ...
  useEffect(() => {
    if (showJoinModal && bill) {
      const taxItems = bill.items
        .filter(
          (item) =>
            item.category?.toLowerCase() === "tax" ||
  ...
  const fetchBill = async () => {
    try {
      ...
      if (result.success && result.bill) {
        ...
        const participant = result.bill.participants.find(
          (p: any) => p.name === savedName,
        );
        if (participant) {
  ...
  // OPTIMISTIC UPDATE - Makes joining feel instant!
  const handleJoinBill = async () => {
    ...
    const optimisticParticipant = {
      ...
      theirShare: 0,
      isPaid: false,
      items: selectedItems.map((id) => ({ itemId: id })),
    };
    ...
    try {
      if (navigator.clipboard && window.isSecureContext) {
        await navigator.clipboard.writeText(
          `${window.location.origin}/bill/${shareId}`,
        );
      } else {
        const textArea = document.createElement("textarea");
        textArea.value = `${window.location.origin}/bill/${shareId}`;
      ...
      prev
        ? {
            ...
            participants: prev.participants.map((p) =>
              p.id === selectedParticipant.id ? { ...p, isPaid: true } : p,
            ),
          }
        : prev,
    );
        ...
        setBill((prev) =>
          prev
            ? {
                ...prev,
                participants: prev.participants.map((p) =>
                  p.id === selectedParticipant.id ? { ...p, isPaid: false } : p,
                ),
              }
            : prev,
        );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="glass rounded-3xl p-8 text-center max-w-sm border border-white/10">
          ...
          <Button
            onClick={() => router.push("/")}
            className="rounded-full w-full bg-gradient-to-r from-indigo-600 to-purple-600"
          >
            Go Home
          </Button>
        </div>
  ...
  const hasCreatorInfo =
    bill.createdBy &&
    bill.createdBy.name &&
    (bill.createdBy.phone || bill.createdBy.defaultUpi);
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-3 py-4">
        ...
        <div className="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-2xl p-6 shadow-2xl mb-4 glow">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="px-5 py-2 bg-white/30 rounded-full backdrop-blur-sm">
        ...
        <div
          className={
            hasCreatorInfo ? "grid lg:grid-cols-3 gap-4" : "max-w-3xl mx-auto"
          }
        >
          <div
            className={hasCreatorInfo ? "lg:col-span-2 space-y-4" : "space-y-4"}
          >
            <div className="glass rounded-2xl border border-white/10">
              <div className="border-b border-white/10 p-4 flex justify-between items-center">
                ...
                {!hasJoined && (
                  <Button
                    ...
                    size="sm"
                    className="rounded-full h-8 bg-gradient-to-r from-indigo-600 to-purple-600"
                  >
              ...
              <div className="p-3 space-y-2">
                {bill.items.map((item) => (
                  <div
                    key={item.id}
                    className="flex justify-between items-center p-3 bg-white/5 rounded-xl border border-white/10 hover:bg-white/10 transition-colors"
                  >
                    <div className="flex-1 pr-2">
                      ...
                      {item.category && (
                        <span className="text-xs bg-indigo-500/30 text-indigo-300 px-2 py-0.5 rounded-full capitalize">
                          {item.category}
                        </span>
                      )}
                    </div>
                    <p className="font-bold text-base text-indigo-400 font-mono">
                ...
                <div className="mt-3 pt-3 border-t border-white/20 flex justify-between items-center p-3 bg-indigo-500/20 rounded-xl">
                  ...
                  <span className="font-bold text-2xl text-indigo-400 font-mono">
                    {formatCurrency(bill.totalAmount)}
                  </span>
                </div>
              </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex justify-between items-center">
                            <p className="font-bold text-lg text-indigo-400 font-mono">
                              {formatCurrency(p.theirShare)}
                            </p>
                            {canPay ? (
                              <Button
                                onClick={() => handlePay(p)}
                                size="sm"
                                className="rounded-full h-8 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700"
                              >
                                <Wallet className="mr-1 w-3 h-3" />
                                Pay
                              </Button>
                            ) : !isMyParticipant && !p.isPaid ? (
                              <div className="text-xs text-gray-500 px-3 py-1 bg-gray-700/50 rounded-full">
                                Pending
                              </div>
                            ) : null}
                          </div>
                ...
                {bill.createdBy!.defaultUpi && (
                  <div className="p-3 bg-indigo-500/20 rounded-xl border border-indigo-500/30">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <button
                        onClick={() =>
                          copyToClipboard(bill.createdBy!.defaultUpi!, "UPI")
                        }
                    ...
                    <p className="font-mono text-indigo-300 font-bold text-xs break-all">
                      {bill.createdBy!.defaultUpi}
                    </p>
                  </div>
                )}
                ...
                {bill.createdBy!.phone && (
                  <div className="p-3 bg-green-500/20 rounded-xl border border-green-500/30">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <button
                        onClick={() =>
                          copyToClipboard(bill.createdBy!.phone!, "Phone")
                        }
                    ...
                    <p className="font-mono text-green-300 font-bold text-xs">
                      {bill.createdBy!.phone}
                    </p>
                  </div>
                )}
        ...
        {showJoinModal && (
          ...
          >
            <motion.div
              ...
              className="glass border-t border-white/20 rounded-t-3xl sm:rounded-3xl p-5 w-full sm:max-w-lg max-h-[85vh] overflow-y-auto"
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-300 mb-2">
                    💡 Taxes are auto-selected for everyone
                  </p>
                      ...
                      return (
                        ...
                        >
                          <div>
                            ...
                            {isTax && (
                              <span className="ml-2 text-xs bg-amber-500/30 text-amber-300 px-2 py-0.5 rounded-full">
                                Auto
                              </span>
                            )}
                          </div>
                ...
                <Button
                  ...
                  disabled={
                    joining || !userName.trim() || selectedItems.length === 0
                  }
                  size="lg"
        ...
        {showParticipantModal && selectedParticipant && (
          ...
          >
            <motion.div
              ...
              exit={{ y: "100%" }}
              transition={{ type: "spring", damping: 25 }}
              className="glass border-t border-white/20 rounded-t-3xl sm:rounded-3xl p-5 w-full sm:max-w-md"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="w-12 h-1 bg-white/30 rounded-full mx-auto mb-4 sm:hidden" />
              <div className="text-center mb-5">
                <div className="w-16 h-16 mx-auto mb-3 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-xl">
                ...
                <p className="text-sm text-gray-400">
                  {selectedParticipant.phone || "No phone number"}
                </p>
                <div className="inline-block px-4 py-2 mt-3 bg-indigo-500/20 rounded-full border border-indigo-500/30">
                  <p className="font-bold text-lg text-indigo-400 font-mono">
                    {formatCurrency(selectedParticipant.theirShare)}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                {selectedParticipant.items.map((pItem: any) => {
                  const item = bill.items.find((i) => i.id === pItem.itemId);
                  if (!item) return null;
                  return (
                    <div
                      key={pItem.itemId}
              ...
              <Button
                onClick={() => setShowParticipantModal(false)}
                variant="outline"
                className="w-full h-12 rounded-xl border-white/20 text-white hover:bg-white/10 mt-4"
              >
                Close
              </Button>
            </motion.div>
              ...
              <div className="space-y-3">
                {bill.createdBy.defaultUpi && (
                  <div className="p-4 bg-indigo-500/20 rounded-xl border border-indigo-500/30">
                    <div className="flex justify-between items-center mb-2">
                      <p className="font-bold text-xs text-indigo-300">
                        UPI ID
                      </p>
                      <button
                        onClick={() =>
                    ...
                    <p className="font-mono text-indigo-200 font-bold text-sm break-all">
                      {bill.createdBy.defaultUpi}
                    </p>
                  </div>
                )}
                ...
                {bill.createdBy.phone && (
                  <div className="p-4 bg-green-500/20 rounded-xl border border-green-500/30">
                    ...
                    <p className="font-mono text-green-200 font-bold text-sm">
                      {bill.createdBy.phone}
                    </p>
                  </div>
                )}
                ...
                <Button
                  ...
                  size="lg"
                  className="w-full h-14 rounded-xl bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 shadow-lg"
                >
